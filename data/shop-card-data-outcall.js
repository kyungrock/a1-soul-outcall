// shop-card-data.js에서 출장마사지 항목만 분리 (서울 전용)
window.outcallShopCardData = [
{
    id: 9,
    name: '20대 인스타이쁜이',
    type: '출장마사지',
    country: 'korea,Thailand,japan',
  region: '서울',
  district: '서울 전지역',
  address: '서울 전지역',
  detailAddress: '서울 내 원하시는 장소(숙소·주거)로 방문 · 사전 상담 시 동선 안내',
    phone: '0507-1859-7033',
    rating: 4.9,
    reviewCount: 0,
    price: '70,000원~',
  description: '서울 전역 홈타이를 기준으로 한 출장 케어입니다. 연락 주시는 동선에 맞춰 방문 일정을 잡고, 스포츠·오일·스웨디시 등 코스별로 몸의 긴장을 풀어 드립니다. 야간·새벽 예약도 상담으로 조율 가능합니다.',
  image: 'images/20대인스타이쁜이.jpg',
  alt: '서울 전지역 출장마사지 20대 인스타이쁜이 — 70,000원~',
    services: ['출장마사지'],
    operatingHours: '24시간 (폰이 꺼진 경우: 마감, 랜덤 휴무)',
    file: 'company-seoul-instapretty-outcall.html',
    showHealingShop: true,
  greeting: '서울 어디서든 홈타이 상담 — 원하시는 시간대를 말씀해 주세요.',
    reviews: [
      {
        author: '히든게스트님',
        rating: 5,
        date: '2025-11-25',
      review: '서울 집에서 홈타이 받았는데 정말 편했어요. 20대 인스타이쁜이 관리사분이 친절하게 와주셔서 감성힐링 스웨디시 받았는데 몸이 정말 편해졌어요. 24시간 운영이라 늦은 시간에도 가능해서 좋았고, 서울 전역이라 어디서든 받을 수 있어서 너무 좋아요.'
      },
      {
        author: '감성여행자님',
        rating: 5,
        date: '2025-11-25',
      review: '서울 집에서 20대 인스타이쁜이 출장마사지 받았어요. VVIP 특급마사지 받았는데 20대 실력파 여 관리사분이 정말 부드럽고 정성스럽게 해주셔서 몸이 한결 가벼워졌어요. 홈타이라 원하는 장소에서 받을 수 있어서 편했고, 꾹꾹 시원 스포츠도 받아봤는데 정말 만족스러웠어요.'
    }
  ]
  },

{
    id: 10,
    name: '한국 20대 이쁜이',
    type: '출장마사지',
    country: 'korea,Thailand,japan',
  region: '서울',
  district: '서울 전지역',
  address: '서울 전지역',
  detailAddress: '서울 홈타이 · 방문 전 요청 사항을 말씀해 주시면 맞춰 준비합니다',
    phone: '0507-1859-6753',
    rating: 4.9,
    reviewCount: 0,
    price: '70,000원~',
  description: '서울 일대 루틴 피로에 맞춘 출장형 힐링입니다. 시내 동선 위주로 이동하며, 가벼운 스트레칭부터 오일·스웨디시까지 단계를 나눠 받을 수 있습니다. 첫 이용이어도 코스 안내를 차분히 도와드립니다.',
  image: 'images/한국20대이쁜이.jpg',
  alt: '서울 전지역 출장마사지 한국 20대 이쁜이 — 70,000원~',
    services: ['출장마사지'],
    operatingHours: '24시간 (폰이 꺼진 경우: 마감, 랜덤 휴무)',
    file: 'company-seoul-korean-pretty-outcall.html',
    showHealingShop: true,
  greeting: '서울 전지역 · 20대 관리 라인 — 부담 없이 코스부터 문의해 보세요.',
    reviews: [
      {
        author: '홈케어러버님',
        rating: 5,
        date: '2025-11-25',
      review: '서울 집에서 한국 20대 이쁜이 출장마사지 받았어요. 꾹꾹 시원 스포츠 코스로 어깨와 목 근육이 확실히 풀렸고, 감성적인 손길이 정말 좋았어요. 집에서 편하게 받을 수 있어서 시간도 절약되고, 20대 관리사분들의 친근한 분위기가 마음에 들었습니다. 다음엔 감미로운 오일마사지도 받아볼게요.'
      },
      {
        author: '밤하늘별님',
        rating: 5,
        date: '2025-11-25',
      review: '서울에서 한국 20대 이쁜이 홈타이 받았어요. 감미로운 오일마사지 받았는데 부드러운 감성 출장마사지로 힐링 가득한 손길을 느낄 수 있어서 정말 만족스러웠어요. 서울 전역이라 어디서든 받을 수 있어서 편리했고, 24시간 운영이라 언제든지 가능해서 좋았습니다.'
    }
  ]
  },

{
    id: 11,
    name: '24시 한국일본혼혈',
    type: '출장마사지',
    country: 'korea,japan,Thailand',
  region: '서울',
  district: '서울 전지역',
  address: '서울 전지역',
  detailAddress: '서울 출장 · 혼혈 테라피 위주 코스, 장소는 사전 협의',
    phone: '0507-1859-6753',
    rating: 4.9,
    reviewCount: 0,
    price: '70,000원~',
  description: '서울을 중심으로 연중 상담 가능한 출장 서비스입니다. 혼혈 라인 특성에 맞춰 부드러운 압과 리듬을 조절하고, 장시간 근무 후 회복용 코스를 추천해 드립니다. 전화 꺼짐 시 휴무일 수 있으니 재문의 부탁드립니다.',
  image: 'images/24시한국일본혼혈.jpg',
  alt: '서울 전지역 출장마사지 24시 한국일본혼혈 — 70,000원~',
    services: ['출장마사지'],
    operatingHours: '24시간 365일 연중무휴',
    file: 'company-seoul-24hour-korean-japanese-outcall.html',
    showHealingShop: true,
  greeting: '24시 상담 가능(유선 상태 기준) — 서울 방문 일정 맞춰 드립니다.',
    reviews: [
      {
        author: '하루쉼표님',
        rating: 5,
        date: '2025-11-25',
      review: '서울에서 24시 한국일본혼혈 출장마사지 받았어요. 24시간 365일 연중무휴라 언제든지 가능해서 좋았고, 혼혈 관리사분이 따뜻한 손길로 감미로운 오일마사지 받았는데 정말 힐링되었어요. 서울 전역이라 접근성도 좋고, 홈타이 서비스라 편안하게 받을 수 있었습니다.'
      },
      {
        author: '늦은밤손님님',
        rating: 5,
        date: '2025-11-25',
      review: '서울 집에서 24시 한국일본혼혈 출장마사지 받았어요. 20대~30대 힐링 출장홈타이 받았는데 따뜻한 손길로 정말 힐링되었어요. 24시간 365일 연중무휴라 언제든지 가능해서 편리했고, 꾹꾹 시원 스포츠와 감성힐링 스웨디시 받았는데 둘 다 만족스러웠어요.'
    }
  ]
  },

{
    id: 12,
    name: '20대 여신 한국인',
    type: '출장마사지',
    country: 'korea',
  region: '서울',
  district: '서울 전지역',
  address: '서울 전지역',
  detailAddress: '서울 출장샵 스타일 방문 · 사전 예약 권장',
    phone: '0507-1859-6946',
    rating: 4.9,
    reviewCount: 0,
    price: '100,000원~',
  description: '서울 일대 VIP 홈케어 스타일의 출장입니다. 한국인 관리사 위주로 구성되어 있으며, 프라이빗한 공간에서 집중 케어를 원하시는 분께 맞춥니다. 스웨디시·프리미엄 위주 상담을 권해 드립니다.',
  image: 'images/VVIP20대여신한국인홈케어.jpg',
  alt: '서울 전지역 출장마사지 20대 여신 한국인 — 100,000원~',
    services: ['출장마사지'],
    operatingHours: '24시간 (폰이 꺼진 경우: 마감, 랜덤 휴무)',
    file: 'company-seoul-vvip-goddess-korean-outcall.html',
    showHealingShop: true,
  greeting: '서울 VIP 홈케어 — 일정·장소는 통화로 확정해 드립니다.',
    reviews: [
      {
        author: '피곤한월요일님',
        rating: 5,
        date: '2025-11-25',
      review: '서울에서 20대 여신 한국인홈케어 출장샵 이용했어요. VIP 스웨디시 받았는데 20대 미녀 관리사분이 정말 부드럽고 정성스럽게 해주셔서 몸이 한결 가벼워졌어요. 출장샵이라 원하는 장소에서 받을 수 있어서 편했고, 서울 전역이라 어디서든 가능해서 좋았습니다.'
      },
      {
        author: '미소천사님',
        rating: 5,
        date: '2025-11-25',
      review: '서울에서 20대 여신 한국인홈케어 출장마사지 받았어요. 어디서나 불러주시면 최대한 빠르게 달려가주셔서 좋았고, 전원 20대 한국인 여 관리사분이 VIP 스웨디시 프로그램으로 최상의 힐링을 선사해주셔서 정말 만족스러웠어요. 서울 전역이라 접근성도 좋았습니다.'
    }
  ]
  },

{
    id: 13,
  name: 'T팬티 테라피 출장',
    type: '출장마사지',
    country: 'korea,Thailand',
  region: '서울',
  district: '서울 전지역',
  address: '서울 전지역',
  detailAddress: '서울 홈타이 · 테마별 코스명 확인 후 방문',
    phone: '0507-1859-6585',
    rating: 4.9,
    reviewCount: 0,
    price: '60,000원~',
  description: '서울 시내 위주로 이동하는 테마형 출장 코스입니다. 건식·스웨디시·믹스 등 단계별 옵션이 다양하니, 체력·시간에 맞게 조합해 보실 수 있습니다. 방문 전 코스명을 알려 주시면 준비가 수월합니다.',
  image: 'images/T팬티콜걸.jpg',
  alt: '서울 전지역 출장마사지 T팬티 테라피 출장 — 60,000원~',
    services: ['출장마사지'],
    operatingHours: '24시간 (폰이 꺼진 경우: 마감, 랜덤 휴무)',
    file: 'company-seoul-tpanty-callgirl-outcall.html',
    showHealingShop: true,
  greeting: '서울 테마 코스 상담 — 원하시는 라인(건식/스웨디시)만 알려 주세요.',
    reviews: [
      {
        author: '리프레시님',
        rating: 5,
        date: '2025-11-25',
      review: '서울 집에서 T팬티 테라피 출장 출장마사지 받았어요. T팬티 본선전 센슈얼스웨디시 받았는데 20대 미녀 관리사분이 정말 짜릿한 손맛으로 해주셔서 너무 만족스러웠어요. 홈타이라 원하는 장소에서 받을 수 있어서 편했고, 서울 전역이라 접근성도 좋았습니다. 다음에도 또 부를 예정이에요.'
      },
      {
        author: '블루문님',
        rating: 5,
        date: '2025-11-25',
      review: '서울에서 T팬티 테라피 출장 출장마사지 받았어요. T팬티 결승전 전신혼합 VVIP 받았는데 전원 20대 실력파 여 관리사분이 정말 최선을 다해 해주셔서 몸이 한결 가벼워졌어요. 단골이 많은곳은 이유가 있다는 걸 느꼈어요. 홈타이라 원하는 장소에서 받을 수 있어서 편리했습니다.'
    }
  ]
  },

{
    id: 14,
    name: '20대 탱글 출장',
    type: '출장마사지',
    country: 'korea,japan,Thailand',
  region: '서울',
  district: '서울 전지역',
  address: '서울 전지역',
  detailAddress: '서울 전역 방문 · 코스 조합은 방문 전 상담',
    phone: '0507-1859-6510',
    rating: 4.9,
    reviewCount: 0,
    price: '70,000원~',
  description: '서울에서 만나는 20대 라인 출장 홈타이입니다. 스포츠성 압과 오일 힐링을 번갈아 선택할 수 있어, 운동 부족·모니터 피로 둘 다 잡고 싶을 때 무난합니다. 믹스·한국인 스웨 등 옵션은 상담 시 안내합니다.',
  image: 'images/20대이쁘니탱글출장.jpg',
  alt: '서울 전지역 출장마사지 20대 탱글 출장 — 70,000원~',
    services: ['출장마사지'],
    operatingHours: '24시간 (폰이 꺼진 경우: 마감, 랜덤 휴무)',
    file: 'company-seoul-pretty-tangle-outcall.html',
    showHealingShop: true,
  greeting: '서울 홈타이 — 스포츠와 오일 중 오늘 몸에 맞는 쪽부터 골라보세요.',
    reviews: [
      {
        author: '스파크님',
        rating: 5,
        date: '2025-11-25',
      review: '서울 집에서 20대 이쁘니 탱글 출장 홈타이 받았어요. 꾹꾹 시원 스포츠와 감성힐링 스웨디시 믹스 코스로 받았는데 20대 실력파 여 관리사분이 정말 부드럽고 정성스럽게 해주셔서 몸이 한결 가벼워졌어요. 서울 전역이라 어디서든 받을 수 있어서 좋고, 홈타이라 편안하게 힐링할 수 있었습니다.'
    }
  ]
  },

{
    id: 15,
    name: '우크라이나출장',
    type: '출장마사지',
    country: 'korea,russia,Thailand',
  region: '서울',
  district: '서울 전지역',
  address: '서울 전지역',
  detailAddress: '서울 홈타이 · 라인·시간대 협의 후 방문',
    phone: '0507-1859-7033',
    rating: 4.9,
    reviewCount: 0,
    price: '70,000원~',
  description: '서울 기준 출발하는 유러피언 감성 라인입니다. 같은 부위를 오래 두드리기보다, 전신 순환에 맞춘 박자로 풀어 드리는 스타일을 지향합니다. 야근 직후·주말 루틴용으로 문의가 많은 편입니다.',
  image: 'images/우크라이나출장.jpg',
  alt: '서울 전지역 출장마사지 우크라이나출장 — 70,000원~',
    services: ['출장마사지'],
    operatingHours: '24시간 (폰이 꺼진 경우: 마감, 랜덤 휴무)',
    file: 'company-seoul-ukraine-outcall.html',
    showHealingShop: true,
  greeting: '서울 출장 — 유러피언 라인 스케줄은 통화 시 안내드려요.',
    reviews: [
      {
        author: '밤하늘별님',
        rating: 5,
        date: '2025-11-25',
      review: '서울 집에서 우크라이나출장 홈타이 받았어요. 감성힐링 스웨디시 받았는데 20대 실력파 여 관리사분이 정말 부드럽고 친절하게 해주셔서 몸이 한결 가벼워졌어요. 서울 전역이라 어디서든 받을 수 있어서 좋고, 24시간 운영이라 늦은 시간에도 가능해서 편리했습니다.'
    }
  ]
  },

{
    id: 16,
    name: '도쿄핫',
    type: '출장마사지',
    country: 'korea,japan',
  region: '서울',
  district: '서울 전지역',
  address: '서울 전지역',
  detailAddress: '서울 홈타이 · 혼혈 관리 위주, 장소는 사전 확정',
    phone: '0507-1859-6415',
    rating: 4.9,
    reviewCount: 0,
    price: '60,000원~',
  description: '서울 시내 동선에 맞춘 혼혈·혼합 라인 출장입니다. 소통 위주로 진행되는 코스가 있어 첫 방문도 부담이 적은 편입니다. VIP·VVIP 등 심화 코스는 시간 여유 있을 때 추천드립니다.',
  image: 'images/출장마사지_도쿄핫.jpg',
  alt: '서울 전지역 출장마사지 도쿄핫 — 60,000원~',
    services: ['출장마사지'],
    operatingHours: '24시간 365일 연중무휴',
    file: 'company-seoul-tokyo-hot-outcall.html',
    showHealingShop: true,
  greeting: '서울 혼혈 라인 — 코스명 말씀 주시면 소요 시간 안내해 드려요.',
    reviews: [
      {
        author: '별빛밤님',
        rating: 5,
        date: '2025-11-25',
      review: '서울 집에서 도쿄핫 출장마사지 받았어요. 동남아 혼혈 관리사분이 정말 친절하고 소통이 잘 되어서 좋았어요. 힘 쿵 코스 받았는데 20대 관리사분이 정말 실력이 좋으셔서 몸이 한결 가벼워졌어요. 서울 전역이라 어디서든 받을 수 있어서 편리했습니다.'
    }
  ]
  },

{
    id: 17,
    name: '원정녀',
    type: '출장마사지',
    country: 'korea,japan',
  region: '서울',
  district: '서울 전지역',
  address: '서울 전지역',
  detailAddress: '서울 홈타이 · 황제/VIP 등 심화 코스는 시간 협의',
    phone: '0507-1859-6415',
    rating: 4.9,
    reviewCount: 0,
    price: '60,000원~',
  description: '서울 방문 위주의 집중 케어형 출장입니다. 짧은 시간에 핵심 부위를 풀어 주는 코스부터 풀 바디 옵션까지 단계가 나뉘어 있으니, 당일 컨디션에 맞게 고르시면 됩니다. 재방문 할인 등은 업체 정책을 확인해 주세요.',
  image: 'images/출장마사지_원정녀.jpg',
  alt: '서울 전지역 출장마사지 원정녀 — 60,000원~',
    services: ['출장마사지'],
    operatingHours: '24시간 365일 연중무휴',
    file: 'company-seoul-wonjeong-outcall.html',
    showHealingShop: true,
  greeting: '서울 출장 — 오늘은 풀코스 vs 부분, 어느 쪽이 편하신지 알려 주세요.',
    reviews: [
      {
        author: '따뜻한차님',
        rating: 5,
        date: '2025-11-25',
      review: '서울 집에서 원정녀 출장마사지 받았어요. 한국계 동남아 혼혈 관리사분이 정말 친절하고 힐링 전문으로 해주셔서 좋았어요. 건강 케어 코스 받았는데 20대 관리사분이 정말 실력이 좋으셔서 몸이 한결 가벼워졌어요. 총알방문이라 빠르게 와주셔서 편리했습니다.'
    }
  ]
  },

{
    id: 18,
    name: '재팬혼혈출장',
    type: '출장마사지',
    country: 'korea,japan',
  region: '서울',
  district: '서울 전지역',
  address: '서울 전지역',
  detailAddress: '서울 출장샵 스타일 · 코스별 소요 분 상담',
    phone: '0507-1859-6632',
    rating: 4.9,
    reviewCount: 0,
    price: '50,000원~',
  description: '서울 일대 1인 위주 혼혈 출장 샵입니다. 타이·아로마·스웨디시 등 아시아계 기법을 섞어 받을 수 있어, 여행·출장 직후 몸을 풀기 좋습니다. 최소 예약 시간은 코스별로 다를 수 있습니다.',
  image: 'images/출장마사지_재팬혼혈.jpg',
  alt: '서울 전지역 출장마사지 재팬혼혈출장 — 50,000원~',
    services: ['출장마사지'],
    operatingHours: '24시간 (폰이 꺼진 경우: 마감, 랜덤 휴무)',
    file: 'company-seoul-japan-mixed-outcall.html',
    showHealingShop: true,
  greeting: '서울 혼혈 1인샵 느낌 — 타이/스웨 중 취향만 알려 주세요.',
    reviews: [
      {
        author: '여유로운하루님',
        rating: 5,
        date: '2025-11-25',
      review: '서울 집에서 재팬혼혈출장 출장샵 이용했어요. 20대 혼혈 관리사분이 정말 예쁘고 부드러운 손길로 타이 코스 받았는데 정말 만족스러웠어요. 언제 어디서든 달려가주셔서 편리했고, 힐링 & 스웨디시도 받아봤는데 최고의 실력으로 해주셔서 몸이 한결 가벼워졌어요.'
    }
  ]
  },

{
    id: 23,
    name: '비키니출장',
    type: '출장마사지',
    country: 'Thailand,japan',
  region: '서울',
  district: '서울 전지역',
  address: '서울 전지역',
  detailAddress: '서울 홈타이 · 일부 구간 제한 있을 수 있음(상담 필수)',
    phone: '0507-1859-7155',
    rating: 4.9,
    reviewCount: 1,
    price: '70,000원~',
  description: '서울 시내 동선을 기준으로 빠른 방문을 지향하는 출장입니다. 타이·아로마·VIP 힐링 등 가벼운 코스부터 긴 코스까지 선택지가 있으며, 일부 동선은 사정상 어려울 수 있으니 상담 시 확인 부탁드립니다.',
  image: 'images/출장마사지_비키니출장.jpg',
  alt: '서울 전지역 출장마사지 비키니출장 — 70,000원~',
    services: ['출장마사지'],
    operatingHours: '24시간 (폰이 꺼진 경우: 마감, 랜덤 휴무)',
    file: 'company-seoul-bikini-outcall.html',
    showHealingShop: true,
  greeting: '서울 출장 — 방문 가능 구간은 통화 시 바로 확인해 드립니다.',
    reviews: [
    {
        author: '야근족님',
        rating: 5,
        date: '2025-11-25',
      review: '야근 후 집에서 비키니출장 출장마사지 받았어요. 비키니 VIP 힐링 코스로 하루 종일 쌓인 피로가 확실히 풀렸어요. 20대 미녀 관리사분의 부드러운 손길이 정말 좋았고, 30분 이내 도착해서 빠른 서비스에 감동했어요. 24시간 운영이라 새벽에도 가능해서 야근족에게 최고의 선택이었습니다.'
    }
  ]
  },

{
    id: 24,
    name: 'VIP 20대 한국홈케어',
    type: '출장마사지',
    country: 'korea',
  region: '서울',
  district: '서울 전지역',
  address: '서울 전지역',
  detailAddress: '서울 홈타이 · 프리미엄 스웨 위주, 일부 지역 제한 가능',
    phone: '0507-1859-6680',
    rating: 4.9,
    reviewCount: 1,
    price: '130,000원~',
  description: '서울 중심 프리미엄 홈케어 출장입니다. 스웨디시 위주의 깊은 이완을 원하시는 분께 맞추었고, 가격대가 높은 만큼 시간·공간 여유를 두고 예약하시는 것을 권합니다. 한국인 관리사 위주 구성입니다.',
  image: 'images/출장마사지_vip20대힐링_한국홈케어.jpg',
  alt: '서울 전지역 출장마사지 VIP 20대 한국홈케어 — 130,000원~',
    services: ['출장마사지'],
    operatingHours: '24시간 연중무휴 (폰이꺼진경우: 마감OR랜덤휴무)',
    file: 'company-seoul-vip-20s-healing-korean-homecare-outcall.html',
    showHealingShop: true,
  greeting: '서울 VIP 홈케어 — 프리미엄 코스는 일정 넉넉히 잡아 주세요.',
    reviews: [
    {
        author: '홈파티호스트님',
        rating: 5,
        date: '2025-11-25',
      review: '집에서 친구들과 모임 전에 VIP 20대 힐링 한국홈케어 출장마사지 받았어요. 스웨디시 A코스로 60분 동안 정말 편안하게 힐링받았고, 20대 한국인 관리사분의 전문적인 기술이 인상적이었어요. 집에서 편하게 받을 수 있어서 시간 절약도 되고, 24시간 운영이라 원하는 시간에 예약할 수 있어서 좋았습니다.'
    }
  ]
  },

{
    id: 25,
    name: '쏘핫',
    type: '출장마사지',
    country: 'korea,japan',
  region: '서울',
  district: '서울 전지역',
  address: '서울 전지역',
  detailAddress: '서울 전역 기준 야간 홈타이 · 일부 지역 상담',
    phone: '0507-1859-7050',
    rating: 4.9,
    reviewCount: 1,
    price: '140,000원~',
  description: '서울 야간대에 강점을 둔 출장입니다. 한국·일본 라인이 섞여 있어 손맛 스타일을 고르실 수 있습니다. 운영이 밤 시간대에 몰려 있으니, 방문 전 반드시 통화 가능 여부를 확인해 주세요.',
  image: 'images/출장마사지_쏘핫.jpg',
  alt: '서울 전지역 출장마사지 쏘핫 — 140,000원~',
    services: ['출장마사지'],
    operatingHours: '오후 8시 ~ 아침 06시 (폰OFF시: 마감OR랜덤휴무)',
    file: 'company-seoul-sohot-outcall.html',
    showHealingShop: true,
  greeting: '서울 야간 출장 — 오늘 밤 시간대만 먼저 알려 주시면 됩니다.',
    reviews: [
    {
        author: '밤샘러버님',
        rating: 5,
        date: '2025-11-25',
      review: '밤늦게 집에서 쏘핫 출장마사지 받았어요. 스웨디시 A코스로 60분 동안 받았는데 20대 미녀 한국&일본인 관리사분의 우아한 손길이 정말 좋았어요. 건강하게 관리해주셔서 다음날 아침 일어나니 몸이 한결 가벼웠고, 오후 8시부터 아침 6시까지 운영이라 야근 후에도 받을 수 있어서 완벽했습니다.'
    }
  ]
  },

{
    id: 26,
  name: '24시 홈케어 힐링',
    type: '출장마사지',
    country: 'korea,japan,Thailand',
  region: '서울',
  district: '서울 전지역',
  address: '서울 전지역',
  detailAddress: '서울 홈타이 · 24시(유선 기준) · 일부 지역 상담',
    phone: '0507-1859-7034',
    rating: 4.9,
    reviewCount: 1,
    price: '70,000원~',
  description: '서울 전역 상담 가능한 24시 부스 스타일 출장입니다. 스포츠·오일·스웨·VVIP까지 폭넓게 운영하며, 당일 컨디션에 따라 강도 조절을 요청하실 수 있습니다. 일부 지역은 협의가 필요합니다.',
  image: 'images/출장마사지_슴살화끈색녀.jpg',
  alt: '서울 전지역 출장마사지 24시 홈케어 힐링 — 70,000원~',
    services: ['출장마사지'],
    operatingHours: '24시간 (폰이꺼진경우: 마감, 랜덤 휴무)',
    file: 'company-seoul-24hour-sexy-outcall.html',
    showHealingShop: true,
  greeting: '서울 24시 — 지금 몸은 뻐근한지, 뻐근+이완 둘 다 필요한지 알려 주세요.',
    reviews: [
    {
        author: '사랑의님',
        rating: 5,
        date: '2025-11-30',
      review: '서울 집에서 24시 홈케어 힐링 출장마사지 받았어요. 감성힐링 스웨디시 A코스 받았는데 20대 힐러 관리사분이 정말 부드럽고 정성스럽게 해주셔서 몸이 한결 가벼워졌어요. 발빠르게 와주셔서 빠른 서비스도 좋았고, 24시간 운영이라 언제든지 받을 수 있어서 편리했습니다.'
    }
  ]
  },

{
    id: 31,
  name: '믹스 스웨 힐링 출장',
    type: '출장마사지',
    country: 'korea,Thailand',
  region: '서울',
  district: '서울 전지역',
  address: '서울 전지역',
  detailAddress: '서울 홈타이 · 한·태 라인 · 일부 지역 제한',
    phone: '0507-1859-6859',
    rating: 4.9,
    reviewCount: 1,
    price: '60,000원~',
  description: '서울 기준 한·태 라인이 섞인 출장입니다. 건식으로 시작해 스웨디시·VVIP로 이어지는 식의 단계형 코스가 특징입니다. 빠른 이동을 내세우는 만큼, 정확한 주소·출입 정보를 미리 주시면 원활합니다.',
  image: 'images/출장마사지_BJ발정난색끼년출장.jpg',
  alt: '서울 전지역 출장마사지 믹스 스웨 힐링 출장 — 60,000원~',
    services: ['출장마사지'],
    operatingHours: '24시간 (폰이꺼진경우: 마감, 랜덤 휴무)',
    file: 'company-seoul-bj-sexy-outcall.html',
    showHealingShop: true,
  greeting: '서울 출장 — 건식으로 시작할지 스웨 위주로 갈지 정해 주세요.',
    reviews: [
    {
        author: '편안한하루님',
        rating: 5,
        date: '2025-11-30',
      review: '서울 집에서 믹스 스웨 힐링 출장 출장마사지 받았어요. 시원한 건식테라피 A코스 받았는데 20대 한국.태국 여 쌤들이 정말 친절하고 따뜻하게 해주셔서 몸과 마음이 동시에 힐링되었어요. 빠른이동으로 빠른 서비스도 좋았고, 24시간 운영이라 언제든지 받을 수 있어서 편리했습니다.'
    }
  ]
  },

{
    id: 33,
  name: '란제리 힐링 출장',
    type: '출장마사지',
    country: 'korea,Thailand',
  region: '서울',
  district: '서울 전지역',
  address: '서울 전지역',
  detailAddress: '서울 전역 홈타이 · 일부 지역은 사전 협의',
    phone: '0507-1859-6648',
    rating: 4.9,
    reviewCount: 1,
    price: '60,000원~',
  description: '서울 일대 섬세한 스웨·건식 조합을 내세운 출장입니다. 행복·힐링을 키워드로 두고, 장시간 앉는 직종 분들의 어깨·허리 라인에 맞춘 상담을 드립니다. 일부 지역은 협의 후 방문합니다.',
  image: 'images/출장마사지_란제리구멍출장.jpg',
  alt: '서울 전지역 출장마사지 란제리 힐링 출장 — 60,000원~',
    services: ['출장마사지'],
    operatingHours: '24시간 (폰이꺼진경우: 마감, 랜덤 휴무)',
    file: 'company-seoul-lingerie-hole-outcall.html',
    showHealingShop: true,
  greeting: '서울 홈타이 — 오늘은 건식으로 풀고 스웨로 마무리할까요?',
    reviews: [
    {
        author: '편안한하루님',
        rating: 5,
        date: '2025-11-30',
      review: '서울 집에서 란제리 힐링 출장 출장마사지 받았어요. 란제리 꾸욱 건식테라피 A코스 받았는데 20대 실력파 여 쌤들이 정말 친절하고 따뜻하게 해주셔서 몸과 마음이 동시에 힐링되었어요. 행복을 전달하는 출장 마사지샵이라 언제나 최선을 다해주셔서 만족스러웠고, 24시간 운영이라 언제든지 받을 수 있어서 편리했습니다.'
    }
  ]
  }
];
